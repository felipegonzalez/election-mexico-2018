...
unavoidable in this setting), a more **consistent behaviour when monitoring partial samples**
...
The idea is that that trustworthy results can be published to the general public early in the counting process.
...
nominal and actual coverage of intervals produced is close. Intervals should be reasonably narrow to be useful in most situations (for example, within one percentage point of actual tallies).
...
A good compromise of the 4 considerations above was obtained by using the following model:
...
persons that can vote in that polling station (there are some polling stations which are of a different type, but we ignore this for now). Let $\theta_{ik}$ the proportion of this list which
...
```{r e2012, warning=FALSE, message=FALSE, cache = TRUE}
...
Add graphs from previous analysis, compared to ratio estimation collapsing strata.
